...
class MDNet(nn.Module):
    ...
    def load_model(self, model_path):
        ...
        # train
        '''
        checkpoint = torch.load("%s" % './'+'RGBT234-RGB-randomseed.pth')['shared_layers']
        #print('checkpoint.keys()',checkpoint.keys())
        model = OrderedDict()
        for i, (key, u) in enumerate(self.layers_v.state_dict().items()):
            #print('self.layers_v.state_dict().items()',i, key)
            model[key] = checkpoint['layers.'+key]
        self.layers_v.load_state_dict(model)
        
        checkpoint = torch.load("%s" % './'+'RGBT234-T-randomseed.pth')['shared_layers']
        model = OrderedDict()
        for i, (key, u) in enumerate(self.layers_i.state_dict().items()):
            model[key] = checkpoint['layers.'+key]
        self.layers_i.load_state_dict(model)
        '''
        #track
